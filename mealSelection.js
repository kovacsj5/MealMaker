//All functions associated with meal plan construction or course swapping
...
function createDay(requiredCourses,coursePool,i){  //coursePool is a list of instances of class course. For now, requiredCourses will always be an empty array. In the future, when I handle leftovers with this app, it will contain some set of meals, likely some of the leftovers or some of the required foods that were requested in the meal plan. i is the marker for the "location" where the day is in the plan. This is specifically for the new UI to place the meals in this day into the correct grid template area
...
function updatePlan(array,button, replacementItem){//this will update one of two global variables: either the "mealPlan" or the "setOfColumnz" after a course was swapped.
    ...
    if(button.kind === "snack"){//there is only one instance of each snack in the entire plan, so there is no need to sift through the entire page to look for more than one instance
        for(i in array[array.length-1]){
            if(i<2){//first two rows of the snacklist are lists of length 2 items each
                for(h in array[array.length-1][i]){
                    if(array[array.length-1][i][h].name === item.name){
                    array[array.length-1][i][h] = replacementItem;
                    }
                }
            }
            else{//for the item in the third row
                if(array[array.length-1][i].name === item.name){
                    array[array.length-1][i] = replacementItem;
                }
            }
            //this will be tricky... Ill have to be mindful of the fact that there could be one or two things in each of the three parts of the last element in mealPlan which is snacks
        }
    }else{
        let j = 0;//starting column
        while (j < array.length - 2){//while column counter is not the snack column
            if(array[j].breakfast.proteinCourse.name === item.name){//all of these sift through each column in the calendar and locate where the item to be replaced is located currently in the calendar
                array[j].breakfast.proteinCourse = replacementItem;
            }else if(array[j].breakfast.veggieCourse.name === item.name){
                array[j].breakfast.veggieCourse = replacementItem;
            }else if(array[j].lunch.proteinCourse.name === item.name){
                array[j].lunch.proteinCourse = replacementItem;
            }else if(array[j].lunch.veggieCourse.name === item.name){
                array[j].lunch.veggieCourse = replacementItem;
            }else if(array[j].dinner.proteinCourse.name === item.name){
                array[j].dinner.proteinCourse = replacementItem;
            }else if(array[j].dinner.veggieCourse.name === item.name){
                array[j].dinner.veggieCourse = replacementItem;
            }
            j++;
        }
    } 
...
function swapCourse(button, availableCourses){
    ...
    updatePlan(mealPlan,button, replacementItem);//changes global variable "mealPlan"
    updatePlan(setOfColumnz,button, replacementItem);//changes global variable "setOfColumnz"
