...
async function initiatePlan(listOfStoreButtons,url){ //Weaves user input together with spreadsheet data to create plan and then renders it. Then returns true to start the course interface function
  ...
  for (i in weeklyBreakdown){
    currentPlan.push(setOfColumnz[weeklyBreakdown[i]]);//pushes class day of each column (including snack column) to the global variable called currentPlan
  }
...
function toggleView(clicked){
    if (clicked.classList.contains('viewOpen')){
        ...
        clicked.nextSibling.remove('viewOpen');
        clicked.nextSibling.add('viewClosed');
        clicked.classList.remove('viewClosed');
...
var currentPlan = [];//this is a global variable that contains the entire set of courses and snacks that have been chosen,
...
planPage.addEventListener('click',function(event){
    ...
    if(clicked.classList.contains('course') || clicked.classList.contains('snack')){
        swapCourse(clicked, coursesThisShop, currentPlan);
    }else if(clicked.classList.contains('nav')){
        toggleView(clicked);
    }
});
